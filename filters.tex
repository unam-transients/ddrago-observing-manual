...
Table~\ref{table:filters} shows typical color terms for the transformations from natural magnitudes to the corresponding magnitudes in the Pan-STARRS and Landolt systems. We transform $gri$ to Pan-STARRS $r$, not to Pan-STARRS $w$, and $z$ and $zy$ to SDSS $z$, not to Pan-STARRS $z$. The color terms are in the sense
\begin{align}
\mbox{Standard magnitude} = \mbox{DDRAGO magnitude} + \mbox{color term}.
\end{align}
For most filters, the color terms are small. The exception is $B$, for which the color term is $+0.24(B-V)$, and $zy$, for which the color term against $z$ is $+0.09(r-i)$. This is not unexpected; matching $B$ is notoriously difficult with CCDs and our $zy$ is expected to be redder than just $z$.
